package org.apache.spark.stanlee.execution.aggregate
...
import org.apache.spark.sql.catalyst.expressions.{Attribute, AttributeReference, AttributeSeq, AttributeSet, BindReferences, BoundReference, EmptyRow, Expression, JoinedRow, Murmur3Hash, NamedExpression, UnsafeProjection, UnsafeRow}
import org.apache.spark.sql.catalyst.plans.physical._
import org.apache.spark.sql.execution._
import org.apache.spark.sql.execution.aggregate.{HashAggregateExec, RowBasedHashMapGenerator, TungstenAggregationIterator, VectorizedHashMapGenerator}
...
/**
  ...
  * 一朝鹏程，快意风云，挥手功名
  */
case class StarryHashAggregateExec(
  ...
  protected override def doExecute(): RDD[InternalRow] = {
    ...
    child.execute().mapPartitionsWithIndex { (partIndex, iter) =>
      createAggIterator(numOutputRows, peakMemory, spillSize, avgHashProbe, aggTime, partIndex, iter)
    }
  }

   def createAggIterator(numOutputRows: SQLMetric, peakMemory: SQLMetric, spillSize: SQLMetric, avgHashProbe: SQLMetric, aggTime: SQLMetric, partIndex: Int, iter: Iterator[InternalRow]) = {
  ...
  private val groupingKeySchema = StructType(groupingAttributes.map(a => StructField(a.name, a.dataType, a.nullable, a.metadata)))
  private val declFunctions = aggregateExpressions.map(_.aggregateFunction)
    .filter(_.isInstanceOf[DeclarativeAggregate])
    .map(_.asInstanceOf[DeclarativeAggregate])
  private val bufferSchema = StructType(aggregateBufferAttributes.map(a => StructField(a.name, a.dataType, a.nullable, a.metadata)))
  ...
  private def doProduceWithKeys(ctx: CodegenContext): String = {
    val initAgg = ctx.addMutableState(ctx.JAVA_BOOLEAN, "initAgg")
    if (sqlContext.sparkSession.sessionState.conf.enableTwoLevelAggMap) {
    ...
    def outputFromVectorizedMap: String = {
      ...
      val generateKeyRow = GenerateUnsafeProjection.createCode(ctx,
        groupingKeySchema.map(f => AttributeReference(f.name, f.dataType, f.nullable, f.metadata)()).zipWithIndex
      ...
      val generateBufferRow = GenerateUnsafeProjection.createCode(ctx,
        bufferSchema.map(f => AttributeReference(f.name, f.dataType, f.nullable, f.metadata)()).zipWithIndex.map { case (attr, i) =>
...
object StarryHashAggregateExec {
  def supportsAggregate(aggregateBufferAttributes: Seq[Attribute]): Boolean = {
    val aggregationBufferSchema = StructType(aggregateBufferAttributes.map(a => StructField(a.name, a.dataType, a.nullable, a.metadata)))
