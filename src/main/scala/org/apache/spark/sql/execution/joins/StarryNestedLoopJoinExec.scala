...
/**
  * Created by passionke on 2018/6/23.
  * 紫微无姓，红尘留行，扁舟越沧溟，何须山高龙自灵。
  * 一朝鹏程，快意风云，挥手功名
  */
case class StarryNestedLoopJoinExec(
                                     left: SparkPlan,
                                     right: SparkPlan,
                                     buildSide: BuildSide,
                                     joinType: JoinType,
  ...
  /**
    ...
    */
  private def innerJoin(relation: List[InternalRow]): RDD[InternalRow] = {
  ...
  /**
    ...
    */
  private def outerJoin(relation: List[InternalRow]): RDD[InternalRow] = {
    streamed.execute().mapPartitionsInternal { streamedIter =>
      val buildRows = relation
      val joinedRow = new JoinedRow
  ...
  private def existenceJoin(relation: List[InternalRow]): RDD[InternalRow] = {
    assert(buildSide == BuildRight)
    streamed.execute().mapPartitionsInternal { streamedIter =>
      val buildRows = relation
      val joinedRow = new JoinedRow
  ...
  /**
    ...
    */
  private def defaultJoin(relation: List[InternalRow]): RDD[InternalRow] = {
  ...
  protected override def doExecute(): RDD[InternalRow] = {
    val relation = SparkPlanExecutor.doExec(broadcast)
    ...
    val resultRdd = (joinType, buildSide) match {
      ...
      case (LeftSemi, BuildRight) =>
        leftExistenceJoin(relation, exists = true)
      case (LeftAnti, BuildRight) =>
        leftExistenceJoin(relation, exists = false)
      case (j: ExistenceJoin, BuildRight) =>
        existenceJoin(relation)
      case _ =>
